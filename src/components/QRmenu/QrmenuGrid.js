...
const QrmenuGrid = () => {
  ...
  const callWaiter = (id1) => {
    ...
    const data = {      
      token: "Bearer ADFLKJDFEREWFEEFSDFSDFDSFSDF"
    };
    setLoading(true);
    axios
  ...
  return (
    ...
    <Row style={{ background: "transparent",   }}>
      ...
      <div style={{background: "#F6FFED", border: "1px solid #c6f598", top: 0, position: "relative", padding: "10px", fontSize: "20px", fontWeight: "bold"}}>
      ЦАХИМ МЕНЮ  -  ALPHA BAR & PUB 
      </div>
      
      <div style={{padding: "10px"}}>
        
      <Button type="primary" loading={false} icon={<BellOutlined />} onClick={(id)=>callWaiter(id)}>
